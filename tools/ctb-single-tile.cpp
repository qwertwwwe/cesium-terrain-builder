...
public:
  ...
  static void
  setTileZ(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))->tileZ = atoi(command->arg);
  }

  static void
  setTileX(command_t *command) {
    static_cast<TerrainBuild *>(Command::self(command))->tileX = atoi(command->arg);
  }
...
buildMesh(const string outputFileName, const MeshTiler &tiler, TerrainBuild *command, bool writeVertexNormals = false) {
  ...
  i_zoom tileZ = command->tileZ;
  int tileX = command->tileX;
  int tileY = command->tileY;
...
main(int argc, char *argv[]) {
  ...
  command.option("-Z", "--tile-z <zoom>", "specify the zoom level to start at. This should be greater than the end zoom level", TerrainBuild::setTileZ);
  command.option("-X", "--tile-x <zoom>", "specify the zoom level to start at. This should be greater than the end zoom level", TerrainBuild::setTileX);
  command.option("-Y", "--tile-y <zoom>", "specify the zoom level to start at. This should be greater than the end zoom level", TerrainBuild::setTileY);
